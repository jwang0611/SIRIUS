...
def ingest_project_kb(
    ...
    session_manager.add_kb_file(session_id, str(shard_path))

    audit = AuditLogger(session_id=session_id)
    audit.log_project_ingest(
        project_name=project_name,
        record_count=len(combined),
        kb_file=str(shard_path),
    )

    logger.info(
        "Project KB ingested: session=%s project=%s rows=%d path=%s",
        session_id,
        project_name,
        len(combined),
        shard_path,
...
def load_session_reference_kb(session_id: str) -> pd.DataFrame | None:
    ...
    if not kb_dir.exists():
        return None
    shards = sorted(kb_dir.glob("project_*.parquet"))
    if not shards:
        return None

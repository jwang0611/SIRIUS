...
class SDTMProcessor(
    ...
    def _process_single_variable_parallel(
        self,
        table_name: str,
        variable_data: dict[str, Any],
        all_variables: list[dict[str, Any]],
    ) -> tuple[str, str, list[dict[str, Any]] | None]:
        """Process a single variable inside the thread-pool.

        In parallel mode, sibling variable *names* are provided for context
        but completed table mappings are not available (they arrive
        concurrently). Sequential mode provides both.
        """
        variable_name = variable_data.get("metadata_variable", "unknown")

        if self._cancel_event.is_set():
            return table_name, variable_name, None

        target_domain: str | None = None
        try:
            # ── Direction 6: NOT_SUBMITTED pre-filter (parallel path) ─────
            if self.kb_hints is not None and self.kb_hints.is_not_submitted_table(
                variable_data.get("annotation_table", "")
            ):
                if self.debug:
                ...
                return table_name, variable_name, not_sub_rec
            ...
            if cascade.recs is not None:
                return table_name, variable_name, cascade.recs
            ...
            if not response_text or response_text.strip() == "":
                return (
                    table_name,
                    variable_name,
                    self._build_fallback_recommendations(table_name, variable_data, target_domain, "AI response empty"),
                )
            ...
            if not json_content or json_content.strip() == "":
                return (
                    ...
                    self._build_fallback_recommendations(
                        table_name, variable_data, target_domain, "AI response missing JSON"
                    ),
                )
            ...
            table_recommendations = validated_result.get("table_recommendations", [])
            if not table_recommendations and isinstance(result, list):
                table_recommendations = result
            ...
            for table_rec in table_recommendations:
                ...
                if (
                    ...
                    raw_domain_recs = table_rec.get("domain_recommendations", [])
                    if raw_domain_recs:
                        max_score = max((r.get("score", 0.0) for r in raw_domain_recs), default=0.0)
                        enforce_domain = not (max_score >= self.domain_override_threshold)
                        domain_recs = self._normalize_domain_recs(
                            table_name=table_name,
                            variable_name=variable_name,
                            domain_recs=raw_domain_recs,
                            target_domain=target_domain,
                            enforce_domain=enforce_domain,
                        )
                        for rec in domain_recs:
                            if "source" not in rec:
                                rec["source"] = "LLM"
                        self._audit_mapping_result(variable_data, domain_recs, cascade_level=4)
                        return table_name, variable_name, domain_recs

            return (
                table_name,
                variable_name,
                self._build_fallback_recommendations(
                    table_name, variable_data, target_domain, "No matching table in LLM response"
                ),
            )

        except Exception as e:
            error_msg = str(e)
            if hasattr(self.client, "get_error_message"):
                error_msg = self.client.get_error_message(e)
            return (
                table_name,
                variable_name,
                self._build_fallback_recommendations(
                    table_name, variable_data, target_domain, f"Exception: {error_msg}"
                ),
            )
    ...
    def _process_mappings_parallel(
        ...
        """Use a thread-pool to process variable mappings in parallel."""
        ...
        for table_name, variable_data, all_variables in table_variable_pairs:
            variable_name = variable_data.get("metadata_variable", "unknown")
            if table_name in all_table_recommendations and variable_name in all_table_recommendations[table_name]:
                continue
            pairs_to_process.append((table_name, variable_data, all_variables))

        if not pairs_to_process:
            return all_table_recommendations, processed_count, was_cancelled

        print(f"🚀 并行处理 {len(pairs_to_process)} 个变量 (max_workers={self.max_workers})...")

        results_lock = threading.Lock()
        completed_count = [skipped_count]

        def update_progress(table_name: str, variable_name: str) -> bool:
            with results_lock:
                completed_count[0] += 1
                current = completed_count[0]

            if progress_callback:
                should_continue = progress_callback(current, total_pairs, table_name, variable_name)
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_pair = {
                executor.submit(self._process_single_variable_parallel, table_name, variable_data, all_variables): (
                    table_name,
                    variable_data,
                )
                for table_name, variable_data, all_variables in pairs_to_process
            }
            ...
            for future in as_completed(future_to_pair):
                ...
                table_name, variable_data = future_to_pair[future]
                variable_name = variable_data.get("metadata_variable", "unknown")

                try:
                    _, _, domain_recs = future.result(timeout=300)
                    ...
                    if domain_recs:
                        ...
                        if not update_progress(table_name, variable_name):
                            was_cancelled = True
                            break
                    else:
                        with results_lock:
                            processed_count += 1
                        update_progress(table_name, variable_name)

                except Exception as e:
                    print(f"❌ 处理 {table_name}.{variable_name} 时发生错误: {e}")
                    fallback = self._build_fallback_recommendations(
                        table_name, variable_data, None, f"Parallel processing error: {e}"
                    )
                    with results_lock:
                        if table_name not in all_table_recommendations:
                            all_table_recommendations[table_name] = {}
                        all_table_recommendations[table_name][variable_name] = fallback
                        processed_count += 1
                    update_progress(table_name, variable_name)
